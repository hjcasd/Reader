...
/**
 * @Author: HJC
 * @Date: 2021/7/29 19:00
 * @Description: 实现流式布局，自定义ViewGroup，实现标签等 - 单行垂直居中 或 水平平分
 */
public class TagFlowLayout extends ViewGroup {
     ...
     * 存储每一行的剩余的空间
     */
    private final List<Integer> lineSpaces = new ArrayList<>();
     ...
     * 存储每一行的view
     */
    private final List<List<View>> lineViews = new ArrayList<>();
        ...
        for (int i = 0; i < columns; i++) {
            ...
            for (int j = 0; j < rows; j++) {
                ...
                if (isAverageInColumn && rows > 1) {
                    averageInColumn = (lineHeight - child.getMeasuredHeight() - params.topMargin - params.bottomMargin) / 2f;
                } else {
                    averageInColumn = 0;
                }
